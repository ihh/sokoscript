...
export default function Tile(props) {
    const { type, state, icon, ...otherProps } = props;
    ...
    if (icon?.rotate && state?.length > 0)
        style.transform = 'rotate(' + (charRotLookup[state.charAt(0)] || 0) + 'deg)';
    return (<Icon icon={name} color={color} style={style} {...otherProps}/>);
}
