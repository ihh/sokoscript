"""End-to-end Treasure Miner RL experiment.

Usage:
    cd python && python -m experiments.miner_rl [--timesteps 1000000]
"""
...
def benchmark_sim_perf(n_steps=100, board_size=16):
    ...
    env = make_treasure_miner_env(board_size, seed=0)
    obs, _ = env.reset()
...
def train_agent(timesteps=1_000_000, board_size=16, n_envs=4, seed=42,
                log_dir='./logs/treasure_miner', lr_schedule='linear'):
    try:
    ...
    def make_env_fn(env_seed):
        def _init():
            return make_treasure_miner_env(board_size, env_seed)
        return _init
    ...
    print(f"\nTraining PPO on treasure_miner for {timesteps} timesteps...")
    t0 = time.time()
    model.learn(total_timesteps=timesteps, callback=callbacks)
    elapsed = time.time() - t0
    print(f"Training complete in {elapsed:.1f}s")

    model_path = os.path.join(log_dir, 'treasure_miner_ppo_final')
...
def main():
    parser = argparse.ArgumentParser(description='Treasure Miner RL experiment')
    parser.add_argument('--timesteps', type=int, default=1_000_000)
    parser.add_argument('--eval-episodes', type=int, default=100)
    parser.add_argument('--board-size', type=int, default=12)
    parser.add_argument('--n-envs', type=int, default=4)
    parser.add_argument('--seed', type=int, default=42)
    parser.add_argument('--log-dir', default='./logs/treasure_miner')
    ...
    print("=" * 60)
    print("Treasure Miner — RL Experiment")
    print("=" * 60)
    ...
    if args.skip_train:
        model_path = args.model or os.path.join(args.log_dir, 'treasure_miner_ppo_final')
    else:
        print("\n--- Training ---")
    ...
    from experiments.miner_heuristic import run_heuristic, run_random
