"""End-to-end Apple Collector RL experiment.

Trains a PPO agent, then compares against heuristic and random baselines.

Usage:
    cd python && python -m experiments.apple_rl [--timesteps 200000] [--eval-episodes 100]
    cd python && python -m experiments.apple_rl --timesteps 1000000 --lr-schedule linear
"""
...
def benchmark_sim_perf(n_steps=100, board_size=16):
    ...
    env = make_apple_collector_env(board_size, seed=0)
    obs, _ = env.reset()
...
def train_agent(timesteps=200_000, board_size=16, n_envs=4, seed=42,
                log_dir='./logs/apple_collector', lr_schedule='constant'):
    """Train PPO agent on Apple Collector."""
    try:
    ...
    def make_env_fn(env_seed):
        def _init():
            return make_apple_collector_env(board_size, env_seed)
        return _init
    ...
    lr_desc = f"linear decay 3e-4→0" if lr_schedule == 'linear' else "constant 3e-4"
    print(f"\nTraining PPO on apple_collector for {timesteps} timesteps with {n_envs} envs (LR: {lr_desc})...")
    t0 = time.time()
    model.learn(total_timesteps=timesteps, callback=callbacks)
    elapsed = time.time() - t0
    print(f"Training complete in {elapsed:.1f}s")

    model_path = os.path.join(log_dir, 'apple_collector_ppo_final')
...
def main():
    parser = argparse.ArgumentParser(description='Apple Collector RL experiment')
    ...
    parser.add_argument('--log-dir', default='./logs/apple_collector', help='Log directory')
    ...
    print("=" * 60)
    print("Apple Collector — RL Experiment")
    print("=" * 60)
    ...
    if args.skip_train:
        ...
        if not model_path:
            model_path = os.path.join(args.log_dir, 'apple_collector_ppo_final')
    ...
    from experiments.apple_heuristic import run_heuristic, run_random
