"""End-to-end Predator-Prey RL experiment.

Usage:
    cd python && python -m experiments.predator_rl [--timesteps 500000]
"""
...
def benchmark_sim_perf(n_steps=100, board_size=16):
    ...
    env = make_predator_prey_env(board_size, seed=0)
    obs, _ = env.reset()
...
def train_agent(timesteps=500_000, board_size=16, n_envs=4, seed=42,
                log_dir='./logs/predator_prey', lr_schedule='constant'):
    try:
    ...
    def make_env_fn(env_seed):
        def _init():
            return make_predator_prey_env(board_size, env_seed)
        return _init
    ...
    print(f"\nTraining PPO on predator_prey for {timesteps} timesteps...")
    t0 = time.time()
    model.learn(total_timesteps=timesteps, callback=callbacks)
    elapsed = time.time() - t0
    print(f"Training complete in {elapsed:.1f}s")

    model_path = os.path.join(log_dir, 'predator_prey_ppo_final')
...
def check_training_health(log_dir):
    ...
    if os.path.exists(eval_path):
        ...
        if np.any(np.isnan(results)) or np.any(np.isinf(results)):
            print("WARNING: NaN/Inf detected in evaluation results!")
            return False
...
def main():
    parser = argparse.ArgumentParser(description='Predator-Prey RL experiment')
    parser.add_argument('--timesteps', type=int, default=500_000)
    parser.add_argument('--eval-episodes', type=int, default=100)
    parser.add_argument('--board-size', type=int, default=16)
    parser.add_argument('--n-envs', type=int, default=4)
    parser.add_argument('--seed', type=int, default=42)
    parser.add_argument('--log-dir', default='./logs/predator_prey')
    ...
    print("=" * 60)
    print("Predator-Prey — RL Experiment")
    print("=" * 60)
    ...
    if args.skip_train:
        model_path = args.model or os.path.join(args.log_dir, 'predator_prey_ppo_final')
    else:
        print("\n--- Training ---")
    ...
    from experiments.predator_heuristic import run_heuristic, run_random
