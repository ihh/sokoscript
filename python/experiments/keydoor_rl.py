"""End-to-end Key & Door RL experiment.

Usage:
    cd python && python -m experiments.keydoor_rl [--timesteps 1000000]
"""
...
def benchmark_sim_perf(n_steps=100, board_size=8):
    from rl.train_ppo import make_key_door_env

    env = make_key_door_env(board_size, seed=0)
    obs, _ = env.reset()
...
def train_agent(timesteps=1_000_000, board_size=8, n_envs=4, seed=42,
                log_dir='./logs/key_door', lr_schedule='linear'):
    try:
    ...
    def make_env_fn(env_seed):
        def _init():
            return make_key_door_env(board_size, env_seed)
        return _init
    ...
    print(f"\nTraining PPO on key_door for {timesteps} timesteps...")
    t0 = time.time()
    model.learn(total_timesteps=timesteps, callback=callbacks)
    elapsed = time.time() - t0
    print(f"Training complete in {elapsed:.1f}s")

    model_path = os.path.join(log_dir, 'key_door_ppo_final')
...
def evaluate_trained(model_path, episodes=100, board_size=8, seed=42):
    from stable_baselines3 import PPO
    from rl.train_ppo import make_key_door_env
...
def main():
    parser = argparse.ArgumentParser(description='Key & Door RL experiment')
    parser.add_argument('--timesteps', type=int, default=1_000_000)
    parser.add_argument('--eval-episodes', type=int, default=100)
    parser.add_argument('--board-size', type=int, default=8)
    parser.add_argument('--n-envs', type=int, default=4)
    parser.add_argument('--seed', type=int, default=42)
    parser.add_argument('--log-dir', default='./logs/key_door')
    ...
    if args.skip_train:
        model_path = args.model or os.path.join(args.log_dir, 'key_door_ppo_final')
    else:
        print("\n--- Training ---")
    ...
    from experiments.keydoor_heuristic import run_heuristic, run_random
