"""Heuristic baseline for the Forest Fire game.

Strategy: move toward the nearest fire (toroidal distance).
If no fires visible, move randomly.

Usage:
    cd python && python -m experiments.fire_heuristic [--episodes 100]
"""
...
def toroidal_direction(x1, y1, x2, y2, size, key_to_action):
    """Return best action to move from (x1,y1) toward (x2,y2) on torus."""
    dx = (x2 - x1) % size
    if dx > size // 2:
        dx -= size
    dy = (y2 - y1) % size
...
def find_cells_by_type(obs, type_indices):
    """Find all cell positions matching any of the given type indices."""
    positions = []
...
def heuristic_action(obs, player_pos, fire_type_indices, board_size, key_to_action):
    """Choose action: move toward nearest fire, or random if none."""
    fires = find_cells_by_type(obs, fire_type_indices)
    if not fires or player_pos is None:
    ...
    best_fire = None
    for fx, fy in fires:
        d = toroidal_distance(px, py, fx, fy, board_size)
        if d < best_dist:
            best_dist = d
            best_fire = (fx, fy)
    ...
    return toroidal_direction(px, py, best_fire[0], best_fire[1], board_size, key_to_action)
...
def run_heuristic(episodes=100, max_steps=500, board_size=16, seed=42, verbose=False):
    ...
    env = make_forest_fire_env(board_size, seed)
    type_names = env._setup_board.grammar['types']
    fire_indices = [i for i, name in enumerate(type_names) if name == 'fire']
    player_idx = next(i for i, name in enumerate(type_names) if name == 'fireman')
    ...
    for ep in range(episodes):
        ...
        while not done:
            ...
            action = heuristic_action(obs, player_pos, fire_indices, board_size, key_to_action)
    ...
    print(f"\nFire Heuristic ({episodes} episodes):")
...
def main():
    parser = argparse.ArgumentParser(description='Forest Fire heuristic baseline')
    ...
    print("=" * 60)
    print("Forest Fire — Heuristic vs Random Baseline")
    print("=" * 60)
